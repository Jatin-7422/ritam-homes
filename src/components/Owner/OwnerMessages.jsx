...
import { Send, User, Loader2, MessageSquare, Trash2, ArrowLeft, Check, CheckCheck, Building } from "lucide-react";
...
export default function OwnerMessages() {
  ...
  useEffect(() => {
    ...
    async function fetchMessagesAndMarkRead() {
      ...
      try {
        ...
        console.error("Error fetching owner chat messages:", err.message);
      } finally {
    ...
    const channel = supabase
      .channel(`owner_chat_${activeChat.property_id}_${activeChat.partner_id}`)
      .on(
        "postgres_changes",
        {
          event: "INSERT",
  ...
  const handleDeleteConversation = async () => {
    if (!window.confirm("Are you sure you want to delete this tenant conversation?"))
      return;
    ...
    <div className="px-4 sm:px-10 py-6 max-w-7xl mx-auto w-full h-[calc(100vh-80px)] flex flex-col">
      <div className="mb-4 flex-shrink-0 flex items-center justify-between">
        ...
        <div className="hidden sm:flex items-center gap-2 bg-[#C5924E]/10 px-3 py-1.5 rounded-xl border border-[#C5924E]/30">
          <Building className="w-4 h-4 text-[#C5924E]" />
          <span className="text-xs font-bold text-[#2D1F1A]">Property Owner Portal</span>
        </div>
      </div>
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-[#E3D9CC] min-h-0">
            {loadingConversations ? (
              ...
              <div className="p-8 text-center text-xs text-[#6E5D53]">
                ...
                No tenant inquiries found for your properties yet.
              </div>
            ) : (
          ...
          {activeChat ? (
            <>
              ...
              >
                <input
                  type="text"
                  placeholder="Reply to prospective tenant..."
                  value={newMessage}
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-[#6E5D53] min-h-0 bg-[#FBF9F4]/50">
              ...
              <strong className="text-sm font-serif text-[#2D1F1A]">
                Select a tenant conversation
              </strong>
              <p className="text-xs text-[#6E5D53] mt-1 max-w-xs">
                Choose an inquiry thread from the left panel to review and reply to prospective clients.
              </p>
            </div>
          )}
        </div>
