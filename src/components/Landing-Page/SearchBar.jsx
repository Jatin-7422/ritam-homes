    ...
    <div className="w-full max-w-6xl mx-auto px-4 my-6">
      ...
      {!isOpen ? (
        ...
        >
          ...
          <div className="z-10 pl-4 sm:pl-5 pr-2 sm:pr-3 flex items-center gap-2 bg-[#2D1F1A] border-r border-[#3E2E27]">
          ...
          <div className="overflow-hidden whitespace-nowrap w-full relative px-2">
            <div className="animate-marquee-slow flex items-center justify-around">
              ...
              {[...phrases, ...phrases, ...phrases, ...phrases].map(
                (item, idx) => (
                  <span
                    key={idx}
                    className="flex items-center gap-3 sm:gap-4 text-xs sm:text-base font-serif italic mx-4 sm:mx-6 text-[#EFEAE1] group-hover:text-[#F3EFE6]"
                  >
                    <span>{item.text}</span>
          ...
          {/* Right Action Hint */}
          <div className="z-10 pr-4 sm:pr-5 pl-2 sm:pl-3 bg-[#2D1F1A] border-l border-[#3E2E27] shrink-0">
            <span className="text-[10px] sm:text-xs font-bold bg-[#3E2E27] text-[#C5924E] px-2.5 sm:px-3 py-1.5 rounded-lg group-hover:bg-[#C5924E] group-hover:text-[#2D1F1A] transition-all whitespace-nowrap">
              Filter ✦
            </span>
          </div>
        </button>
        ...
        /* EXPANDED SEARCH BAR */
        <div className="bg-[#FAF7F2] p-4 sm:p-5 rounded-2xl border border-[#EADBCE] shadow-2xl relative transition-all animate-in fade-in zoom-in-95 duration-200">
          {/* Close / Collapse Button */}
          <button
            onClick={() => setIsOpen(false)}
            className="absolute -top-3 -right-3 bg-[#2D1F1A] text-white p-1.5 rounded-full shadow-md hover:bg-[#C5924E] transition-colors cursor-pointer z-20"
            title="Close Search"
          >
            <X className="w-4 h-4" />
          </button>
          ...
          <form
            ...
            className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 items-center w-full"
          >
            {/* Location Input */}
            <div className="bg-white p-3 rounded-xl border border-[#E3D7C8] flex flex-col justify-center w-full">
            ...
            <button
              type="submit"
              className="w-full h-full py-3.5 px-6 bg-[#2D1F1A] text-white rounded-xl font-bold text-xs flex items-center justify-center gap-2 hover:bg-[#3E2E27] transition-all shadow-md active:scale-95 cursor-pointer sm:col-span-2 lg:col-span-1"
            >
