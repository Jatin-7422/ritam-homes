...
export default function Login({ setIsLoggedIn }) {
  ...
  // Signup Modal State with Confirm Password and Role Option
  ...
  const [signupData, setSignupData] = useState({
    ...
    password: "",
    confirmPassword: "",
    role: "user", // Added role state with default "user"
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Set a flag indicating they just logged in and need to make a choice
      sessionStorage.setItem("show_intent_popup", "true");

      // Redirect to the main landing page/dashboard route
      ...
      const { data, error } = await supabase.auth.signUp({
        ...
        options: {
          data: {
            ...
            role: signupData.role, // Added role saved to user metadata
          },
        },
      });
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fadeIn overflow-y-auto">
            <div className="relative w-full max-w-md bg-[#3b2219] border border-[#d4af37]/30 rounded-3xl p-6 sm:p-8 shadow-2xl text-white my-8">
              ...
              <form onSubmit={handleSignupSubmit} className="space-y-3.5">
                ...
                {/* Added Role Selection Option */}
                <div>
                  <label className="block text-xs font-semibold text-white/80 mb-1">Account Role</label>
                  <div className="relative">
                    <User className="absolute left-3.5 top-3 w-4 h-4 text-white/40" />
                    <select
                      name="role"
                      ...
                      onChange={handleSignupChange}
                      className="w-full bg-black/30 border border-white/20 rounded-xl px-4 py-2.5 pl-10 text-xs text-white focus:outline-none focus:border-[#d4af37]"
                    >
                      <option value="user" className="bg-[#3b2219] text-white">User</option>
                      <option value="admin" className="bg-[#3b2219] text-white">Admin</option>
                    </select>
                  </div>
                </div>
